...
import allImg from "../../../public/products/all.jpg";
      ...
      <div className="max-w-6xl mx-auto relative flex items-center justify-center">
        ...
        <div className="relative min-h-[480px] w-full mt-2 md:-mt-6">
          {slides.map((item, i) => (
            ...
            >
              <div
                className={`flex flex-col md:flex-row items-center justify-between gap-12 md:gap-20 w-full px-4 md:px-0 ${i % 2 !== 0 ? "md:flex-row-reverse" : ""
                  }`}
              >
                {/* Image Section */}
                <div className="relative group w-full md:w-[48%] md:h-[420px] h-[380px] rounded-3xl overflow-hidden shadow-2xl">
                  <img
                    src={item.image}
                    alt={item.title}
                    className="w-full h-full object-cover object-top rounded-3xl transform transition-all duration-700 ease-in-out group-hover:scale-110 group-hover:brightness-110"
                    loading="lazy"
                  />
                </div>
