...
const subCategoryImages = {
  ...
  Men: {
    "All Products": "/products/mencategory.jpg",
    Topwear: "/products/mentopwear.jpg",
    Bottomwear: "/products/menbottomwear.jpg",
    Winterwear: "/products/menwinterwear.jpg",
  },
  Women: {
    "All Products": "/products/womencategory.jpg",
    Topwear: "/products/womentopwear.jpg",
    Bottomwear: "/products/womenbottomwear.jpg",
    Winterwear: "/products/womenwinterwear.jpg",
  },
  Kids: {
    "All Products": "/products/kidscategory.jpg",
    Topwear: "/products/kidstopwear.jpg",
    Bottomwear: "/products/kidsbottomwear.jpg",
    Winterwear: "/products/kidswinterwear.jpg",
  },
};
...
const Category = () => {
  ...
  const filterProducts = (autoClose = true) => {
    let filtered = productsData.filter((p) => p.category === capitalize(main));

    if (filteredSubCategories.length > 0) {
      filtered = filtered.filter((p) => filteredSubCategories.includes(p.subCategory));
    } else if (currentSub !== "All Products") {
    ...
    if (selectedAvailability.length > 0) {
      filtered = filtered.filter((p) => {
        if (selectedAvailability.includes("inStock")) return true;
        if (selectedAvailability.includes("outOfStock")) return false;
        return true;
      });
    }
  ...
  }, [selectedSizes, filteredSubCategories, priceRange, selectedAvailability]);
  ...
  if (!main) return <PageNotFound />; 
  if (baseProducts.length === 0 && !loading) return <PageNotFound />;
  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      <nav className="container mx-auto px-4 py-3 text-sm text-gray-600 flex flex-wrap items-center gap-1 md:mt-8 mb-2">
        ...
        <NavLink to={`/${main.toLowerCase()}`} className="hover:underline ml-1">
          {capitalize(main)}
        </NavLink> &gt;
        <span className="ml-1 font-semibold">{currentSub}</span>
      </nav>
        ...
        {subCategories.slice(0, 4).map((subCat, idx) => {
          ...
          return (
            ...
            >
              ...
              <span className="text-xs md:text-sm text-center font-medium mb-1">{subCat}</span>
            </NavLink>
          );
        })}
      </div>
        ...
        >
          ...
          <div className="flex-1 overflow-y-auto py-4 space-y-6 hide-scrollbar-custom px-5">
            ...
            <div className="flex gap-3 border-b border-gray-200 pb-5">
              <span className="text-lg font-semibold text-gray-700">{capitalize(main)}</span>
              <span className="text-lg text-gray-500">({totalCategoryProducts})</span>
            </div>
